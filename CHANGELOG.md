...
- **Sessions** : un cookie émis avant un changement de mot de passe ou une (re)configuration MFA est refusé. Le changement de mot de passe délivre un nouveau cookie. Une date `passwordUpdatedAt` / MFA `confirmedAt` **présente mais invalide** refuse la session (fail closed).
...
Aucune migration SQL. Session « Rester connecté » toujours 60 jours. `style-src` garde `unsafe-inline` (CSS Tailwind / Google Fonts).
...
- **Natel (≤ 760 px)** : carte étroite (~236 px / 68 vw) et vitrée ; le fond n’est plus en `cover` (qui recadrait pistons et voiture) mais en `118% auto` pour montrer l’illustration autour du login.

### Conservé

Fichier `og-v3.png` pour l’aperçu social. Aucune migration SQL.
...
- **Maj + Entrée** : retour à la ligne **dans** le même bloc (Entrée continue de créer un nouveau bloc). Coller garde les sauts ; une ligne vide dans le collage crée un nouveau bloc.
